...
Game::Game(unsigned int width, unsigned int height, const char* title)
	:window(sf::VideoMode(width, height), title), level(nullptr), nextLevel(nullptr), willExit(false)
{
	instance = this;
}
...
Game* Game::getInstance(unsigned int width, unsigned int height, const char* title)
{
	if(!instance)
	{
